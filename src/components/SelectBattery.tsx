...
import { Battery, ArrowLeft, History } from 'lucide-react';
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { supabase } from '@/lib/supabase';
import { useAuth } from '@/lib/auth-provider';
import { useDevice, useAvailableBatteries, invalidateQueries } from '@/lib/hooks';
...
export function SelectBattery({ deviceId }: SelectBatteryProps) {
  ...
  const { user } = useAuth();
  const { device, batteries: installedBatteries, loading: deviceLoading } = useDevice(deviceId);
  const { availableBatteryGroups, loading: batteriesLoading } = useAvailableBatteries(device?.battery_type ?? '');
  const [selectedBatteries, setSelectedBatteries] = useState<Array<{ groupId: string; batteryId: string; status: 'charged' | 'in_use' | 'empty' | 'disposed' }>>([]);
    ...
    if (installedBatteries.length > 0) {
      setSelectedBatteries(
        installedBatteries.map(battery => ({
          ...
          status: battery.status as 'charged' | 'in_use' | 'empty' | 'disposed',
        }))
      );
    }
  }, [installedBatteries]);
  ...
  const handleToggleBattery = (groupId: string, batteryId: string, status: 'charged' | 'in_use' | 'empty' | 'disposed') => {
    ...
    try {
      ...
      if (selectedBatteries.length > 0) {
        const { error: updateError } = await supabase
          ...
          .in('id', selectedBatteries.map(b => b.batteryId));
      ...
      // キャッシュを無効化
      const { invalidateBatteries, invalidateDevices } = invalidateQueries(queryClient);
      await Promise.all([
        invalidateBatteries(),
        invalidateDevices(),
      ]);
  ...
  // 電池グループを名前でソート
  const sortedBatteryGroups = [...availableBatteryGroups].sort((a, b) => 
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden mb-6">
          <div className="px-4 py-5 sm:px-6 bg-gray-50">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {device.name}に設定する電池を選択してください（最大{device.battery_count}本）
            </p>
          </div>
          ...
          {sortedBatteryGroups.length === 0 ? (
            <div className="p-6 text-center">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">利用可能な電池がありません</h3>
              <p className="mt-1 text-sm text-gray-500">
            ...
            <div className="divide-y divide-gray-200">
              {sortedBatteryGroups.map((group) => {
                // すべての電池を表示（設置済みの電池も含む）
                const batteries = group.batteries?.sort((a, b) => a.slot_number - b.slot_number) ?? [];
                  ...
                  <div key={group.id} className="p-6">
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                      {batteries.map((battery) => {
                        const isSelected = selectedBatteries.some(b => b.batteryId === battery.id);
                        const isInstalled = battery.device_id !== null;
                        const isInstalledInOtherDevice = isInstalled && battery.device_id !== device.id;
                        const isDisabled = !isSelected && (
                          selectedBatteries.length >= device.battery_count || isInstalledInOtherDevice
                        );

                        return (
                          <button
                            key={battery.id}
                            onClick={() => !isInstalledInOtherDevice && handleToggleBattery(group.id, battery.id, battery.status as 'charged' | 'in_use' | 'empty' | 'disposed')}
                            disabled={isDisabled}
                          ...
                          >
                            <div className="flex-1">
                              ...
                              <div className="mt-1">
                                <BatteryStatusBadge status={battery.status as 'charged' | 'in_use' | 'empty' | 'disposed'} />
                              </div>
